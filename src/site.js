...
window.WonderSneak = (function () {
  ...
  return { OUT: OUT, park: park, run: run, hide: hide };
})();
(function(){
...
/* The footer robot peeks round the moon, and keeps peeking while the footer is
   in view: out of sight, a beat, then the sneak in, a hold at home, and again.
   The mark paints after the font loads, so parking has to wait for geometry;
   the old version parked once at script time, lost the race and never ran. */
(function () {
  ...
  if (window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches) return;
  var seen = false, busy = false, timer = null;
  var HOLD = 3200, GONE = 700;
  function cycle() {
    if (!seen || busy || document.hidden) return;
    busy = true;
    WonderSneak.hide(cv, 480, function () {
      timer = setTimeout(function () {
        WonderSneak.run(cv, 1700, function () { busy = false; timer = setTimeout(cycle, HOLD); });
      }, GONE);
    });
  }
  function arm() {
    var io = new IntersectionObserver(function (es) {
      es.forEach(function (e) {
        ...
        if (seen && !busy) {
          clearTimeout(timer);
          busy = true;
          timer = setTimeout(function () { WonderSneak.run(cv, 1700, function () { busy = false; timer = setTimeout(cycle, HOLD); }); }, 220);
        }
        if (!seen) { clearTimeout(timer); }
      });
    }, { threshold: 0.4 });
    io.observe(cv);
  }
  var tries = 0;
  (function park() {
    if (WonderSneak.park(cv)) { arm(); return; }   // geometry exists: hold it off-stage
    if (++tries < 400) requestAnimationFrame(park); // ~6s at 60fps, then give up quietly
  })();
})();
