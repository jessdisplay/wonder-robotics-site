...
window.WonderSneak = (function () {
  ...
  return { OUT: OUT, park: park, run: run };
})();
(function(){
...
/* The rails drift on their own (Jesse, 14 Sep 2026: "these can scroll
   without click"). They used to ride the page scroll, which meant a rail only
   moved while the visitor was moving, and a visitor who stopped to look saw
   it stop too. Now a rail in view travels at a slow constant speed, holds a
   beat at each end and comes back, so it is always alive and never lost.
   A hand on it pauses it (hover, focus, touch); a drag, a sideways wheel or
   an arrow hands the rail to the visitor for good. Off under reduced motion.
   Snap and smooth CSS are lifted only while we drive, because both fight a
   scrollLeft that changes every frame. */
(function () {
  var rails = [].slice.call(document.querySelectorAll('.rail-scroll'));
  if (!rails.length) return;
  if (window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches) return;
  var SPEED = 26;      // px per second
  var HOLD = 1800;     // ms at each end
  function take(rail) { rail._manual = true; rail.style.scrollSnapType = ''; rail.style.scrollBehavior = ''; }
  rails.forEach(function (rail) {
    rail._dir = 1; rail._hold = 0; rail._paused = false; rail._x = rail.scrollLeft;
    ...
    rail.addEventListener('pointerenter', function () { rail._paused = true; });
    rail.addEventListener('pointerleave', function () { rail._paused = false; });
    rail.addEventListener('focusin', function () { rail._paused = true; });
    rail.addEventListener('focusout', function () { rail._paused = false; });
    document.querySelectorAll('[data-rail="' + rail.id + '"]').forEach(function (b) {
  ...
  function frame(now) {
    ...
    rails.forEach(function (rail) {
      ...
      if (r.bottom < 0 || r.top > vh) return;            // out of view: rest
      var max = rail.scrollWidth - rail.clientWidth;
      if (max <= 0) return;
      if (rail._paused || document.hidden) return;
      if (rail._hold > 0) { rail._hold -= dt; return; }
      rail.style.scrollSnapType = 'none';
      rail.style.scrollBehavior = 'auto';
      rail._x += rail._dir * SPEED * dt / 1000;
      if (rail._x >= max) { rail._x = max; rail._dir = -1; rail._hold = HOLD; }
      if (rail._x <= 0) { rail._x = 0; rail._dir = 1; rail._hold = HOLD; }
      rail.scrollLeft = rail._x;
    });
...
(function () {
  ...
  if (!cv || !window.WonderSneak || !('IntersectionObserver' in window)) return;
  if (window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches) return;
  if (!WonderSneak.park(cv)) return;              // hold it off-stage until seen
  var io = new IntersectionObserver(function (es) {
    es.forEach(function (e) {
      if (e.isIntersecting) { io.disconnect(); setTimeout(function () { WonderSneak.run(cv, 1700); }, 220); }
    });
  }, { threshold: 0.4 });
  io.observe(cv);
})();
