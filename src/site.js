...
(function(){
  ...
  var MACHINES=[
    ...
    // Add-ons. The Moton list prices these inside the formats, never on their
    // own, so they are quoted, not guessed: a null price reads "On quote" and
    // they never count as machines for install or maintenance.
    {id:'icem', name:'Ice maker',        model:'BTB Z02, add-on',             kit:'Supplied on every coffee format. Add a second, or one to a line that has none', price:null},
    {id:'prnt', name:'Chocolate and caramel printer', model:'Yingmei, add-on', kit:'Supplied on B Pro, B Standard and Smart EFF. Add it to the bartender', price:null},
    {id:'milk', name:'Extra milk line',  model:'Add-on',                      kit:'A second milk, oat or soy, on any barista format', price:null},
    {id:'syr',  name:'Extra syrup channels', model:'Add-on',                  kit:'Beyond the three channels supplied', price:null},
    {id:'brand',name:'Branding',          model:'Wonder, scoped',              kit:'Mark, colours, cups, bags, menu, signage. One system across the machine and the room', price:null},
    {id:'eng',  name:'Engineering and fit-out', model:'Wonder, scoped',        kit:'Counter, cell, guarding, services, extraction, the drawings, the build', price:null},
    {id:'soft', name:'Software',          model:'Wonder, scoped',              kit:'Ordering, payment, the screen, the dashboard', price:null}
  ];
  ...
  MACHINES.forEach(function(m){
    ...
    tr.innerHTML='<td class="name">'+m.name+'<small>'+m.model+'</small></td><td class="kit">'+m.kit+'</td>'+
      '<td class="price num">'+(m.price==null?'On quote':money.format(m.price))+'</td>'+
  ...
  function render(){
    ...
    MACHINES.forEach(function(m){
      ...
      if(n>0){
        if(m.price==null){open.push(m.name.toLowerCase()); lines.push(line(m.name+', '+m.model, n+' \u00d7 on quote', null));}
        else{count+=n; supply+=n*m.price; lines.push(line(m.name+', '+m.model, n+' \u00d7 '+money.format(m.price)+', supply', n*m.price));}
      }
    });
  ...
  // No pick, and the sheet opens on the kitchen pair as before.
  var pick=(location.search.match(/[?&]pick=([^&]*)/)||[])[1];
  if(pick){ pick.split(',').forEach(function(id){ if(id in qty) qty[id]=1; }); }
  else{ qty.noo=1; qty.fry=1; }
  render();
})();
