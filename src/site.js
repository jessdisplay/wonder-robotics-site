...
window.WonderQuote = (function(){
  ...
  // What makes a package: any machines, plus the whole job around them
  // (brand, website, branding on the machine, fit-out, software, install)
  // and a year of care. The ready-made packages are starting points that name
  // themselves when the machines are exactly theirs; the container kitchen
  // only when it was chosen, since it carries a build of its own. Anything
  // else with the whole job is the buyer's own package, and saves the same.
  function packageFor(qty,svc,pkgId){
    var mine=MACHINES.filter(function(m){return (qty[m.id]||0)>0;});
    if(!mine.length||!PACKAGE_WORK.every(function(w){return svc[w];})) return null;
    ...
    var exact=PACKAGES.filter(function(p){ return !p.extra&&covers(p)&&mine.every(function(m){return !!p.machines[m.id];}); })[0];
    if(exact) return exact;
    return {id:'own',own:true,name:'Your package',line:'The machines and the whole job around them, from one team.',img:mine[0].img,
            machines:mine.reduce(function(o,m){o[m.id]=qty[m.id];return o;},{})};
  }
  ...
  function compute(st){
    ...
    var pk=noPlan?null:packageFor(qty,svc,st.pkg);
    var held=!!pk;
    var save=0;
    if(held){
...
/* The quote page: a quote shown the way a product page shows a product.
   The name over the picture, the price set large with its sums, then every
   part as its own spread. Built in the sheet; this is where it lands. The
   state is the address, so the page is a link, and the proposal is one
   click from it. */
(function(){
  ...
  function draw(){
    ...
    var ref='WR-Q'+String(now.getFullYear()).slice(2)+('0'+(now.getMonth()+1)).slice(-2)+('0'+now.getDate()).slice(-2)+'-'+(c.count+c.asks);
    var title=c.pkg&&!c.pkg.own?c.pkg.name:Q.titleOf(c.lines);
    var hero=c.pkg?c.pkg.img:(famImg(picked[0].id)||picked[0].img);
    var q=Q.toQuery(st);
    ...
    var sums='<dt>Machines at list</dt><dd>'+money.format(c.supply)+'</dd><dt>The work and care</dt><dd>'+money.format(work)+'</dd>';
    if(c.pkg) sums+='<dt>'+esc(c.pkg.name)+', 10% off the work</dt><dd class="save">'+money.format(-c.save)+'</dd>';
    if(del) sums+='<dt>Delivery</dt><dd>'+val(del)+'</dd>';
    sums+='<dt>GST 10%</dt><dd>'+money.format(c.gst)+'</dd><dt>Total inc GST</dt><dd>'+money.format(c.inc)+'</dd>';
    var body=['Quote '+ref+': '+title].concat(c.lines.map(function(l){ return l.label+': '+val(l); }))
      .concat(['Total ex GST: '+money.format(c.sub),'Total inc GST: '+money.format(c.inc),'',location.origin+location.pathname+q]).join('\n');
    ...
    var options='<div class="qv-opts">'+
      '<div><span class="label">Maintenance</span><div class="qv-segs">'+opt('plan','standard','Standard',c.plan==='standard')+opt('plan','priority','Priority',c.plan==='priority')+opt('plan','none','None',c.plan==='none')+'</div></div>'+
      (c.plan!=='none'?'<div><span class="label">For</span><div class="qv-segs">'+[1,2,3,4,5].map(function(t){return opt('term',t,t+(t===1?' year':' years'),c.term===t);}).join('')+'</div></div>':'')+
      '<div><span class="label">Delivery</span><div class="qv-segs">'+opt('site','port','Within 100 km of a port',c.site==='port')+opt('site','far','Further',c.site==='far')+'</div></div></div>';

    var spreads=parts.map(function(l,i){
      var d=Q.DETAILS[l.id]||null, img=(l.id==='care'?(Q.DETAIL_IMG.care||l.img):l.img);
      if(l.id==='care'&&d) d={stage:d.stage,gets:d.gets,lead:'Looked after: spares, monitoring and updates, and a number that answers when something stops.',facts:[['Plan',(c.plan==='priority'?'Priority':'Standard')+', '+c.term+(c.term===1?' year':' years')],['After that','Priced by the year']]};
      var stage=d?d.stage:(l.kind==='extra'?'Build':'Supply');
      ...
      var note=l.kind==='machine'?'List, ex GST':l.value===null?'Priced to order':'Indicative, ex GST';
      var gets=d?'<div><span class="label">What you get</span><ul class="inc-gets">'+d.gets.map(function(g){return '<li>'+esc(g)+'</li>';}).join('')+'</ul></div>':'';
      var facts=d?'<dl class="inc-facts">'+d.facts.map(function(f){return '<dt>'+esc(f[0])+'</dt><dd>'+esc(f[1])+'</dd>';}).join('')+'</dl>':'';
      return '<article class="inc'+(i%2?' flip':'')+'"><div class="inc-h"><span class="inc-i">('+('0'+(i+1)).slice(-2)+')</span><span class="inc-stage">'+esc(stage)+'</span>'+
        '<h3>'+esc(name)+'</h3><div class="inc-price'+(l.value===null?' ask':'')+'">'+val(l)+'<small>'+note+'</small></div></div>'+
        '<figure class="inc-ph"><img src="'+img+'" alt="" loading="lazy" style="object-position:'+pos(img)+'"></figure>'+
        '<div class="inc-b"><p class="inc-lead">'+esc(lead)+'</p>'+((gets||facts)?'<div class="inc-cols">'+gets+facts+'</div>':'')+'</div></article>';
    }).join('');

    host.innerHTML=
      '<section class="phero"><div class="wrap"><div class="pills"><span class="pill">Quote '+ref+'</span><span class="pill">Valid until '+long.format(until)+'</span><span class="pill">About two months</span><span class="pill price">'+(c.indicative?'About ':'')+money.format(c.sub)+' ex GST</span></div>'+
      ...
      '<section class="pnum"><div class="wrap"><span class="label">[ The whole job, one price ]</span>'+
        '<div class="pn-fig">'+money.format(c.sub)+'</div>'+(c.pkg&&c.pkg.extra?'<span class="pn-extra">plus '+esc(c.pkg.extra.name.toLowerCase())+', priced on scope</span>':'')+
        '<div class="pn-row"><p>'+(c.pkg?esc(c.pkg.line)+' Bought as one job, the work costs 10% less.':'Machines at list. The work around them is indicative until we have seen the site.')+(c.asks?' Robots are priced to order.':'')+'</p><dl class="pn-sum">'+sums+'</dl></div>'+
        options+
      ...
      '<section class="rail last"><div class="wrap"><span class="label">[ Terms ]</span><div><ol class="hl three">'+
        ...
        '<li><b>Valid 30 days</b><p>Until '+long.format(until)+'</p></li><li><b>One year warranty</b><p>On the machines</p></li>'+
        '<li><b>About two months</b><p>From order, duty unpaid within 100 km of a port</p></li><li><b>Terms of sale</b><p><a class="link" href="'+root+'terms/">Read them</a></p></li></ol></div></div></section>';
  ...
  host.addEventListener('click',function(e){
    ...
    var k=b.getAttribute('data-opt'), v=b.getAttribute('data-val');
    if(k==='term') st.term=parseInt(v,10); else st[k]=v;
...
(function(){
  ...
  // choices the sheet does not show but must not lose when a quote page is changed in it
  var carry={plan:'standard',term:1,site:'port'};
  ...
  function packageHolds(){ return Q.packageFor(qty,svc,pkg); }
  ...
  function resultHtml(){
    ...
    // The package creator. Whatever machines were chosen, show what makes
    // it a package, which of those pieces are on, and what finishing it saves.
    var banner='', heldP=packageHolds();
    if(count>0&&heldP){
      var hc=Q.compute({qty:qty,parts:parts,svc:svc,pkg:pkg});
      banner='<div class="wz-pkg on"><div><span class="label">'+(heldP.own?'Your package':'The whole job')+'</span><h4>'+(heldP.own?'This is your package.':'This is '+esc(heldP.name.toLowerCase())+'.')+' You save '+money.format(hc.save)+'.</h4>'+
        '<p>The whole job around the machines, a year of maintenance included, and the work costs 10% less.</p></div></div>';
    }else if(count>0){
      var trySvc={}; Q.SERVICES.forEach(function(x){ trySvc[x.id]=svc[x.id]; }); Q.PACKAGE_WORK.forEach(function(w){ trySvc[w]=true; });
      var tryPkg=(wz.where==='box'&&qty.fry>0&&qty.noo>0)?'box':pkg;
      var tc=Q.compute({qty:qty,parts:parts,svc:trySvc,pkg:tryPkg});
      var pieces=Q.PACKAGE_WORK.map(function(w){ var x=Q.SERVICES.filter(function(s){return s.id===w;})[0]; return '<li class="'+(svc[w]?'on':'')+'">'+esc(x.name)+'</li>'; }).join('')+'<li class="on">A year of maintenance</li>';
      var missing=Q.PACKAGE_WORK.filter(function(w){return !svc[w];}).length;
      banner='<div class="wz-pkg make"><div><span class="label">Make it a package</span><h4>Add '+(missing===1?'one more piece':missing+' more pieces')+' and save '+money.format(tc.save)+'.</h4>'+
        '<p>Any machines become a package when we do the whole job around them. The work costs 10% less.</p><ul class="wz-pieces">'+pieces+'</ul></div>'+
        '<button type="button" class="btn" data-kind="makepkg" data-id="'+(tryPkg||'')+'"><span>Make it my package</span><i aria-hidden="true">+</i></button></div>';
    }
  ...
  $('wz').addEventListener('click',function(e){
    ...
    else if(k==='makepkg'){ Q.PACKAGE_WORK.forEach(function(w){ svc[w]=true; }); if(id) pkg=id; }
  ...
  function render(){
    ...
    if(held){
      ...
      if(!held.own) pick.push('pkg='+held.id);
    }
    var ul=$('qs-lines');
    ...
    if(!things){ totalEl.textContent=money.format(0); noteEl.textContent='Pick a machine to start'; }
      ...
      noteEl.textContent=held?((held.own?'Your package':held.name)+', you save '+money.format(save)+'. Ex GST'):bits.join(', ')+'. Ex GST';
    }
    ...
    if(carry.plan!=='standard') qs.push('plan='+carry.plan); if(carry.term!==1) qs.push('term='+carry.term); if(carry.site==='far') qs.push('site=far');
